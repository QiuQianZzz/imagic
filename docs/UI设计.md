...
1. 开关行：「启动时检查更新」+ 副标题「启动 Imagic 时自动检查新版本」（Switch.adaptive）
2. 同一行：「更新渠道」标签居左 + `SegmentedButton<UpdateChannel>` 两段居右（正式版 / 所有版本），默认选中正式版
3. 说明文字：根据当前安装形式动态显示——绿色版显示「自动替换文件并重启」，安装版/MSIX 显示「启动安装向导，由您完成剩余步骤」
4. 按钮行：`FilledButton.icon`「检查更新」（检查中禁用）右侧内联状态文本：
   - 检查中：14px 圆环 spinner + 「正在检查更新...」
   - 已是最新：次要色文本
   - 发现新版本：primary 文本「发现新版本 vX.Y.Z」+「查看」TextButton
   - 失败：error 色文本展示原因

**更新对话框**（`AlertDialog`，圆角 20）：

```
┌── 发现新版本 0.2.0 ────────────────────┐
│  当前版本 0.1.0 → 新版本 0.2.0        │
│  更新日志                              │
│  ┌──────────────────────────────────┐ │
│  │  ### 新增                        │ │  ← maxHeight 240 滚动
│  │  - ...                           │ │
│  └──────────────────────────────────┘ │
│  [================----]  62%          │  ← 下载进度 LinearProgressIndicator
│                                        │
│  [稍后]                    [下载并更新] │
└────────────────────────────────────────┘
```

- 下载中显示线性进度与百分比；校验中显示「正在校验文件完整性（SHA-256）...」；失败显示 error 色原因
- 「下载并更新」点击后下载并校验，成功后应用退出，由脚本替换文件并重启
